...
Entry is `main.ts`, loaded inside wp-admin. Built by Vite (see root `CLAUDE.md`) into `live-sandbox-editor/build/main.js` (entry, ~10 KB), `monaco-[hash].js` (shared chunk holding monaco-editor's eager core, ~3 MB), language-mode chunks (lazy), and `monaco.css`. Only `main.js` is enqueued as an ES module via `wp_enqueue_script_module()`; the other chunks are loaded transitively by relative URL.

## Modules

- `main.ts` — sets up the Monaco worker environment, then calls `initApp(root)`. Nothing else belongs here.
- `monaco-environment.ts` — installs `self.MonacoEnvironment` and creates Vite-managed Monaco workers. Imported by `main.ts` first so `MonacoEnvironment` is set before any `monaco-editor` module evaluates.
...
- Monaco web workers are wired in `monaco-environment.ts` using Vite's `?worker` imports. `main.ts` imports it before `app.ts`, so `MonacoEnvironment` is set before any `monaco-editor` module evaluates.
- `main.js` must not contain monaco-editor's eager core. WordPress enqueues `main.js?ver=…` while Vite's lazy chunks (cssMode, tsMode, …) import their shared parent by bare relative URL (no `?ver`). If both URLs target the same file, the browser treats them as different modules and re-runs `main.js`'s side effects (second editor + Playground iframe). `vite.config.mts` enforces this with `manualChunks: { monaco: ['monaco-editor'] }` — keeps the eager core in `monaco-[hash].js`, which both `main.js` and the lazy chunks reference.
- Let Vite own worker URL resolution; do not hand-register workers in PHP or pass worker URLs through `AppData`.
