...
- Monaco web workers are configured via `self.MonacoEnvironment.getWorker` in `editor.ts` using Vite's `?worker` imports. Swapping bundlers requires replicating this.
